...
- **自由画笔研究（GH #56,#59~#72；`?freehand` 演示路由，与主应用正交、main 仍用组件/图元引擎）**：新范式"画作=运笔笔触 + 逐笔动画"。向心 CR 中心线 + 变宽墨带(perfect-freehand) + 弧长运笔 + 笔尖动画(#56/#60)；`sceneToStrokes` 组件场景转笔触重绘(#59)；Rough.js 手绘抖动默认关可选(#61/#62)；恒宽/离屏/调速/过程=结果(#63~#66)；着色定为沿形向心等高线流水填、实色涂满无矩形(#67~#72)。结论：引擎补曲线/构图，弃生图(丢可编辑)与组件库(覆盖封顶+同质化)。

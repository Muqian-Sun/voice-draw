...
**vpath 连接/贴附件（手/脚/耳/帽/轮/翅膀）一律用上面的 at 关系定位，别把它们的位置烤进 d 绝对坐标**——
手→袖/臂末端用 {"ref":{"byName":"左袖"},"anchor":"bottom","onEdge":true}、脚→腿末端、耳→头 onEdge；
让引擎按父件**真实边缘**贴（你最不擅长盲算这种像素坐标，交引擎算才准、改起来也不脱节）。
这类件的 d 只画**局部形状**（如手画在 0 附近的小团），落点全交给 at。

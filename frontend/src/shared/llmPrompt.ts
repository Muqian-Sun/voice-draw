...
- **五官/细节精致（通用准则，非教某种动物长相）**：正面或四分之三视角、对称器官**一律 mirror**；五官与细节件用**细描边(strokeWidth 2~3)或无描边**、忌粗黑团；眼睛**大而有神**+白色高光点(同样 mirror 出对称)更灵动；鼻/嘴小巧、器官间距协调不挤不散；可加腮红点缀，达绘本/贴纸插画水准。
- **头发/帽子/头巾别实心盖住脸**：发型画成沿脸侧与头顶的**镂空轮廓/环形**、露出正脸区域；按 z 序**先画头/脸底 → 再画头发(贴脸侧或脑后) → 五官放最上层**，让眼/鼻/嘴始终可见。一开始就把脸留出来，别画完发现盖住了再反复 zorder 救场（调层救不了实心色块压脸）。
- **画人/人形角色（人物/公主/小矮人/玩偶等）的结构准则**（人形最易画丑，务必按此）：
  ① **绘本卡通比例：头大、头身比约 1:3~1:4**，别画成瘦长真人；
  ② 部件 z 序后→前：**脑后一大片有体积的头发** → 躯干(连衣裙/上衣下身) → **双臂双腿(mirror 对称)** → 脚和鞋 → 脸 → 额前刘海(框额头两侧、但露出眉眼) → 五官放最上层 → 腮红/高光；**头发必须明显画出来、别画成秃头**；
  ③ 脸画**圆润椭圆**，五官集中在脸中下部、**眼睛大带白色高光**、鼻小嘴小、左右用 mirror 对称、间距协调不挤；
  ④ **四肢俱全别省**：双手、双脚/鞋都要有，对称件一律 mirror；
  ⑤ 忌：头太小 / 五官挤歪 / 头发盖脸 / **秃头缺头发** / 缺胳膊少腿 / 方块僵硬——轮廓一律 C 贝塞尔曲线圆润流畅，达绘本贴纸级。
...
5. mode=plan 时（创作拆解）：**按场景复杂度给足 op，别为省 op 牺牲完整度**——
   单主体 8~26 个；**多主体场景（如「白雪公主和七个小矮人」「一家三口」）按每个主体 6~10 op 分配、整体最多 ~50 个**，
   且**先把一个主体画完整（五官+身体+四肢俱全、能认出是谁）再画下一个**，绝不把 8 个角色挤进 24 笔画成各 3 笔的残缺轮廓；
   从背景到前景、从大到小；
   ...
   **造型必须符合对象本身**：动物=四足横向身体（横向躯干 + 四条腿向下 + 头在前端一侧 + 耳朵 + 尾巴），
   **绝不要画成直立堆叠的人形**——只有人、雪人才竖直堆叠；鸟有翅膀、鱼为横向纺锤、车为横向带轮、树竖直；
   **范例（尤其例6 vpath 猫）只示范画法（vpath 部件拆分 / 相对定位 / mirror 对称 / 铺满背景），不是要照搬的模板**——
   你本就清楚各种事物长什么样，画什么就按它本来的样子自行设计部件与比例，勿照搬范例坐标；
   **蓬松外形（羊毛/树冠/云/鬃毛）用 vpath 的 C 曲线描成连绵起伏的轮廓，别用一堆圆并排堆**；
...
[例6] mode=plan，utterance="画一只猫"（**vpath 插画范例**：每部件一条命名 vpath、C 曲线描轮廓、对称件用 mirror、五官细描边+眼高光；动物=四足横向、头在前端、**非直立人形**。背景用图元矩形铺满；只示范画法，按对象本来样子自行设计、勿照搬坐标）
